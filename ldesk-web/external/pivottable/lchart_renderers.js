	...
	callWithJQuery(function ($) {
		...
		makeGoogleChart = function (chartType, extraOptions) {
			return function (pivotData, opts) {
				...
				if (opts.gchart.chartType) {
					chartType = opts.gchart.chartType;
				}
				...
				var id = "";
				if (opts.gchart.chartId) {
					id = "id=" + opts.gchart.chartId;
				}
				...
				if (opts.gchart.showEdit) {
					result.bind("dblclick", function () {
						var editor;
						editor = new google.visualization.ChartEditor();
						google.visualization.events.addListener(editor, 'ok', function () {
							var options = editor.getChartWrapper().getOptions();
							chartType = editor.getChartWrapper().getChartType();

							if (opts.gchart.onEdit) {
								opts.gchart.onEdit(options, chartType);
							}
		...
		return $.pivotUtilities.lchart_renderers = {
			...
			"Donut Chart": makeGoogleChart("PieChart", { pieHole: 0.4 })
		};
	});
